...
# Fetch the expression data for these genes from the `myeloid` Seurat object:
expression_data <- combined_data@assays$RNA@data[genes_of_interest,]

# Extract metadata to group by "cell_type" and "type":
metadata <- combined_data@meta.data[, c("cell_type", "type")]

# Function to calculate mean expressions for a given gene
calculate_mean <- function(gene_name) {
  gene_expression <- expression_data[gene_name,]
  combined_df <- data.frame(Gene_Expression=gene_expression, metadata)
  
  mean_values <- combined_df %>%
    group_by(cell_type, type) %>%
    summarise(mean_expression = mean(Gene_Expression, na.rm = T)) %>%
...
# Convert table to a grid object
table_plot <- tableGrob(clean_table)

# Save the grid object as PNG
png("clean_table.png", width=650, height=750)  # Adjust width and height as necessary
