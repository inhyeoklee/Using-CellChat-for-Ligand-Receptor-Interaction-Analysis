...
genes_to_test_myeloid <- c("SPP1", "MIF")
genes_to_test_lymphoid <- c("CD44", "CD74", "CXCR4")

# Function to perform differential expression for each cell type
perform_DE_for_each_cell_type <- function(seurat_obj, genes_to_test) {
  unique_cell_types <- unique(seurat_obj$cell_type)
  all_results <- list()
  
  for (current_cell_type in unique_cell_types) {
    # Subset the Seurat object for the current cell type
    subset_data <- subset(seurat_obj, subset = cell_type == current_cell_type)
    
    # Perform differential expression between carotid and femoral
    markers <- FindMarkers(subset_data, group.by = "type", ident.1 = "carotid", ident.2 = "femoral", features = genes_to_test)
    
    # Store the results in the list
    all_results[[current_cell_type]] <- markers
  }
...
# Perform differential expression for each cell type in myeloid dataset
myeloid_results <- perform_DE_for_each_cell_type(myeloid, genes_to_test_myeloid)

# Perform differential expression for each cell type in lymphoid dataset
lymphoid_results <- perform_DE_for_each_cell_type(lymphoid, genes_to_test_lymphoid)
...
library(ggpubr)
# As an example for the MIF plot in myeloid cells
p5 + geom_signif(comparisons = list(c("Carotid", "Femoral")), test = "wilcox.test", map_signif_level = TRUE)
